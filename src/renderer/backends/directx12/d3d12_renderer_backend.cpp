...
GraphicsAPI D3D12RendererBackend::getGraphicsAPI() const {
...
bool D3D12RendererBackend::initWindowContext() {
    return true;
}

bool D3D12RendererBackend::init() {
...
bool D3D12RendererBackend::createSwapChain(void* hwnd) {
    ...
    HRESULT hr = factory->CreateSwapChainForHwnd(commandQueue, (HWND)hwnd, &swapChainDesc, nullptr, nullptr, &swapChain1);
    factory->Release();
...
bool D3D12RendererBackend::createDepthStencil() {
    ...
    if (FAILED(device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &depthDesc,
        D3D12_RESOURCE_STATE_DEPTH_WRITE, &clearValue, IID_PPV_ARGS(&depthStencil)))) return false;
    
    device->CreateDepthStencilView(depthStencil, nullptr, dsvHeap->GetCPUDescriptorHandleForHeapStart());
    return true;
}
...
bool D3D12RendererBackend::createCommandObjects() {
    ...
    return SUCCEEDED(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator, nullptr, IID_PPV_ARGS(&commandList)));
}
...
bool D3D12RendererBackend::createConstantBuffers() {
    ...
    for (int i = 0; i < 3; i++) {
        if (FAILED(device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufferDesc,
            D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&constantBuffers[i])))) return false;
...
void D3D12RendererBackend::clear() {
    ...
    if (mainCamera) {
        auto& bg = mainCamera->getBackgroundColor();
        clearColor[0] = bg.r; clearColor[1] = bg.g; clearColor[2] = bg.b; clearColor[3] = bg.a;
    }
...
void D3D12RendererBackend::draw(const Mesh& mesh) {
    ...
    D3D12_VERTEX_BUFFER_VIEW views[2] = {*d3d12Buffer->getVertexBufferView(), *d3d12Buffer->getNormalBufferView()};
...
void D3D12RendererBackend::setUniforms(void* shaderProgram) {
    ...
    if (mainCamera) {
        glm::mat4 model = glm::rotate(glm::mat4(1.0f), SDL_GetTicks() / 1000.0f, glm::vec3(0.5f, 1.0f, 0.0f));
    ...
    for (int i = 0; i < 3; i++) {
        commandList->SetGraphicsRootConstantBufferView(i, constantBuffers[i]->GetGPUVirtualAddress());
    }
}
...
void D3D12RendererBackend::renderSkybox(const Mesh& mesh, unsigned int shaderProgram, unsigned int textureID) {
}
